...
var Client = {
	...
	rename : function(elem){
		...
		$.post('process.php',{type:'rename', elem : elem.name, name : name, path:Client.path.get()},function(data){
			...
			// Update Client
			var id = elem.name;
			var tmp = Client.contents[id];
			delete Client.contents.id;
			// Update name
			tmp.name = name;
			Client.contents[name] = tmp;
			// Update UI
			$('#'+id.replace(/-_-_/g,' ')).find('.name').text(name);
		},'json');
	},
	del : function(elem){
		...
		$.post('process.php',{type:'delete', elem : elem.name, path:Client.path.get()},function(data){
			...
			delete Client.contents.id;
			// Update UI
			console.log('#'+id.replace(/-_-_/g,' '))
			console.log($('#'+id.replace(/-_-_/g,' ')).parents('.item'))
			$('#'+id.replace(/-_-_/g,' ')).parents('.item').remove();
		},'json');
	}
};