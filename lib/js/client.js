...
var Client = {
	...
	getDir : function(dir, callback){
		...
		if(!Client.isCached(dir)){
			console.log('Fetching from AJAX directory='+dir);
			// We need to build local cache, send AJAX petition
			$.post('process.php',{type:'list',dir:dir},function(data){
				if(!data.done){
	...
	drawDir : function(dir){
		...
		$.each(Client.contents, function(k,file) {
			...
			console.log(file)
			if(file.parent !== dir) return true;
	...
	back : function(num){
		...
		UI.move(Client.loaded.view, Client.current.view)
		// Reset loaded
	...
	move : function(elem,dest){
		...
		// Let's move!
		$.post('process.php',{type:'move',elemName:elem.name, destName:dest.name},function(data){
			if(!data.done){
				alert('An error ocurred ['+data.msg+']');
				UI.removeLoaders();
				return true;	
			...
			$('a#'+elem.name).parents('.item').remove();
