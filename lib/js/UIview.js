...
var UI = {
	...
	icons : {
		...
		// Change icon size
		size : function(size){
			// That size will be the width, height will be +30
			if(size<20) return true; // Minimum width
			this.width = size;
			this.height = size+size*0.35;
			this.preview = this.height-14;
			$('.item a').css({
				width : UI.icons.width,
				height : UI.icons.height
			});
			$('.item a .file .preview').css({
	...
	drawItem : function(file, dom){
		...
		if(file.type==1){
			...
			if(ext=='txt' || ext == 'doc' || ext == 'docx'){
				extra += '<div class="preview" style="height:'+UI.icons.preview+'">Lorem ipsum dolor sit amet, consectetur adipiscing elit. In sit amet leo eget ligula suscipit dictum.<br /><br />Sed tincidunt sagittis leo, id interdum urna egestas vel.</div>';	
			}
		...
		dom.append('<div class="item"><a href="'+href+'" id="'+file.name+'" rel="'+file.type+'" class="'+classes+'" style="width:'+UI.icons.width+'px; height:'+UI.icons.height+'px"><div class="icon '+type+'" style="'+style+'">'+extra+'</div><div class="name">'+file.name+'</div></a></div>');
	},
