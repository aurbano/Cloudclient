...
var UI = {
	...
	init : function(){
		...
		setTimeout(function(){ $('#uploadHelp').fadeOut(); },2000);
		// Track mouse
		...
		$('#viewport').bind("contextmenu", function(event) {
			...
			Client.fileOpID = null;
			$('#fileOps').hide();
			$('#pasteOp').hide();
			if(Client.clipboard.elem!==null) $('#pasteOp').show();
			var elem = document.elementFromPoint(UI.mouse.x, UI.mouse.y) // x, y
			$('.item').removeClass('under');
			if($(elem).hasClass('icon') || $(elem).hasClass('name') || $(elem).attr('href')=='#open' || $(elem).hasClass('type') || $(elem).hasClass('preview')){
				// Select the parent:
			...
			if(selectedItem){
				$('#fileOps').show();
				into = $('.under a').attr('id').replace(/-_-_/g,' '); // ID fixes
				console.log('Selected '+into);
				Client.fileOpID = into;
				$('#pasteOp').hide();
			}
			$("#context-menu").css({top: event.pageY + "px", left: event.pageX + "px"}).fadeIn(200);
		});
		// Remove context menu
		...
		// -------------------- File Ops ---------------------
		$("a[href='#fileOps']").click(function(event){
			event.preventDefault();
			$("#context-menu").fadeOut(200);
			var action = $(this).attr('rel');
			var id = Client.fileOpID;
			console.log('Perform '+action+' on '+id);
			switch(action){
				case 'rename':
					Client.rename(Client.contents[Client.path.get()+id]);
					break;
				case 'delete':
					Client.del(Client.path.get()+id);
					break;
				case 'copy':
					Client.copy(Client.contents[Client.path.get()+id]);
					break;
				case 'cut':
					Client.cut(Client.contents[Client.path.get()+id]);
					break;
				case 'paste':
					Client.paste();
					break;
	...
	// Hide views and show Loading text
	// for complex operations that will change everything! 
	...
	// Display bgLoader div
	bgLoader : function(state){
		if(state) return $('#bgLoader').fadeIn();
		$('#bgLoader').fadeOut();
	},
	...
	actions : function(){
		...
		(function(event){
			...
			if($(this).attr('rel')==0){
				...
				console.log('Path: '+Client.path.get()+', file: '+Client.contents[Client.path.get()+dir].name);
				window.location = '/file/'+Client.path.get()+Client.contents[Client.path.get()+dir].name;
			}
		});
		...
		$('.item').draggable({
			...
			start: function(event, ui) {
				...
				if(Client.contents[Client.path.get()+UI.dragging].icon=='folder_public'){
			...
			drag : function(event,ui){
				...
				if((Client.contents[Client.path.get()+UI.dragging].icon=='folder_user' && Client.contents[Client.path.get()+into].icon=='folder_user') || Client.contents[Client.path.get()+UI.dragging].icon=='folder_public'){
					$('.item').removeClass('under');
					return true;
				}
			},
			stop : function(event,ui){
				...
				console.log('Moving '+Client.contents[Client.path.get()+UI.dragging].name+' into '+Client.contents[Client.path.get()+into].name);
				if(UI.dragging == into){
