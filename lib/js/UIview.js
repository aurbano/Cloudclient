...
var UI = {
	...
	icons : {
		size : 128,
		// Translates files properties to icon src
		src : function(file){
			// Requires all file properties
			if(file.type==0){
				// Folder
				var name = '';
				switch(file.icon){
				...
				return 'places/tlag-light-folder'+name+'.png';
			}else{
				var name = file.mime;
				name = name.replace('/','-');

				return 'mimetypes/'+name+'.png';
			}
		}
	},
	init : function(){
	...
	adjust : function(){
		...
		this.viewport.h = height-headerHeight;
		this.viewport.w = width-sidebarWidth;
		// Update sidebar
		$('#sidebar').height(height-headerHeight);
		// Update viewport
		$('#viewport').height(this.viewport.h);
		$('#viewport').width(this.viewport.w);
		// Update views
		$('.view').height(this.viewport.h);
		$('.view').width(this.viewport.w);
	},
	...
	drawItem : function(file, dom){
		//dom.append('<div class="item"><a href="#open" id="'+file.id+'" rel="'+file.type+'"><div class="icon"><img src="img/icons/'+UI.icons.size+'x'+UI.icons.size+'/'+UI.icons.src(file)+'" width="'+UI.icons.size+'" height="'+UI.icons.size+'" /></div><div class="name">'+file.name+'</div></a></div>');
		var type = 'folder';
		var classes = '';
		var extra = '';
		var style = '';
		...
		if(file.type==1){
			...
			if(ext=='txt' || ext == 'doc' || ext == 'docx'){
				extra += '<div class="preview">Lorem ipsum dolor sit amet, consectetur adipiscing elit. In sit amet leo eget ligula suscipit dictum.<br /><br />Sed tincidunt sagittis leo, id interdum urna egestas vel.</div>';	
			}
		...
		if(file.mime!==null){
			...
			if(mime[0] == 'image'){
				...
				style += 'background:url(\'/thumb/'+Client.path.get()+Client.contents[file.parent].name+'/'+file.name+'\') no-repeat center !important; box-shadow:none;';
				classes += ' image';
				href = '/file/'+Client.path.get()+Client.contents[file.parent].name+'/'+file.name;
			}
		}
		
		dom.append('<div class="item"><a href="'+href+'" id="'+file.id+'" rel="'+file.type+'" class="'+classes+'"><div class="icon '+type+'" style="'+style+'">'+extra+'</div><div class="name">'+file.name+'</div></a></div>');
	},
	// Move views in the screen
	// Both params must be jQuery DOM objects
	move : function(oldView, newView){
		console.log('Move views (Show old/new)');
		console.log(oldView);
		console.log(newView);
		UI.loading(0);	
		if(oldView!==null){
	...
	actions : function(){
		...
		$('.item').draggable({
			...
			start: function(event, ui) {
				$(this).addClass('drag');
				UI.dragging = $(this).children('a').attr('id');
