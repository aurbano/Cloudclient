...
var UI = {
	...
	init : function(){
		...
		$("a[href='#fileOps']").click(function(event){
			...
			switch(action){
				case 'rename':
					Client.rename(Client.contents[id]);
					break;
				case 'delete':
					Client.del(Client.contents[id]);
					break;
				case 'copy':
					Client.copy(Client.contents[id]);
				case 'cut':
					Client.cut(Client.contents[id]);
					break;
				default:
					alert('Not ready yet!');	
			}
	...
	actions : function(){
		...
		(function(event){
			...
			if($(this).attr('rel')==0){
				...
				console.log('Path: '+Client.path.get()+', file: '+Client.contents[dir].name);
				window.location = '/file/'+Client.path.get()+Client.contents[dir].name;
			}
		});
